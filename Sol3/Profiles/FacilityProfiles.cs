...
namespace Sol3.Profiles
{
    ...
    public class TankDTOValidator : AbstractValidator<TankDTO>
    {
        ...
        {
            RuleFor(x => x.Name).Length(3, 50).WithMessage($"{nameof(TankDTO.Name)} должен быть от 3 до 5 символов.");
    ...
    public class UnitDTOValidator : AbstractValidator<UnitDTO>
    {
        ...
        {
            RuleFor(x => x.Name).Length(3, 50).WithMessage($"{nameof(UnitDTO.Name)} должен быть от 3 до 5 символов.");
